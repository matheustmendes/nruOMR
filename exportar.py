...
Uso:
    python exportar.py <scan.pdf> <config.yaml> <planilha_alunos.xlsx> <aba> [--merge scan2.pdf scan3.pdf]

Exemplos:
    python exportar.py scan.pdf config_canela.yaml alunos.xlsx "CANELA IMPRESSÃO"
    python exportar.py scan1.pdf config_canela.yaml alunos.xlsx "CANELA IMPRESSÃO" --merge scan2.pdf scan3.pdf
"""
...
def processar_pdf_completo(paginas, config, pagina_inicio=1):
    ...
    print(f"\nResumo: {paginas_processadas} páginas processadas, {paginas_brancas} em branco puladas")
    print(f"Total de alunos lidos: {len(todos_resultados)}")
...
def contar_presencas(resultados, dias):
    ...
    Returns:
        ...
        {
            "numero": 1,
            "presencas": 3,  # total de dias presente na semana
            "detalhes": {
...
def exportar_xlsx(contagem, alunos, dias, arquivo_saida):
    ...
    for i, c in enumerate(contagem):
        ...
        for dia in dias:
            d = c["detalhes"][dia]
            if d["presente"]:
                # Mostra A, J ou AJ dependendo do que foi marcado
                marcas = ""
                if d["almoco"]:
                    marcas += "A"
                if d["janta"]:
...
def aplicar_correcoes(resultados, correcoes):
    ...
    Args:
        resultados: lista de dicts do ler_bolhas
        correcoes: dict do correcoes.json
    """
    alterados = 0
    for r in resultados:
        for dia in r["dias"]:
...
def main():
    ...
    if "--merge" in sys.argv:
        ...
        # Pega todos os args depois de --merge até o próximo flag ou fim
        extras = []
