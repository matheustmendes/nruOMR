...
def classificar_cantos(candidatos: list, largura_img: int, altura_img: int,
                       distancia_maxima: float = 300) -> dict:
    """
    Identifica qual marcador pertence a qual canto da imagem.

    Cada marcador é atribuído ao canto mais próximo dele, desde que esteja
    dentro de distancia_maxima pixels do canto. Candidatos mais distantes são
    ignorados — o canto fica ausente e será estimado por completar_marcadores_faltantes.
    Isso evita que bolhas preenchidas (detectadas no fallback de solidez) sejam
    confundidas com marcadores de canto quando o marcador real não foi detectado.

    Args:
        candidatos: lista de (cx, cy, area, solidez)
        largura_img: largura da imagem em pixels
        altura_img: altura da imagem em pixels
        distancia_maxima: distância máxima (px) do canto para aceitar candidato

    Returns:
        dict com chaves dos cantos detectados (pode ter menos de 4)
    """
    if len(candidatos) < 1:
        dica = (
            "Dicas:\n"
    ...
    for nome, (rx, ry) in cantos_ref.items():
        ...
        if melhor is not None and menor_dist <= distancia_maxima:
